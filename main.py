...
# ---------------------------------------------------------------------------
# Zoom — Peace sign, up = zoom in, down = zoom out (Ctrl+scroll)
# ---------------------------------------------------------------------------
...
def main():
    global zot_prev_pos, scroll_prev_pos, zoom_prev_pos
    ...
    try:
        while cap.isOpened():
            ...
            for i, line in enumerate([
                "4: index+middle+ring+pinky — scroll",
                "Peace: index+middle — zoom (up=in, down=out)",
                "Zot: index+pinky, thumb+mid+ring pinched — cursor",
            ]):
                cv2.putText(frame, line, (10, h - 50 + i * 20),
